                ...
                for ($i = 0; $i < count($request->option1); $i++) {
                    ...
                    for ($j = 0; $j < count($request->option2); $j++) {
                        VariantOption::create([
                            ...
                            'stock' => $request->stock[$countStockOrPrice],
                            'price' => $request->price[$countStockOrPrice],
                        ]);
                        $total_stock += $request->stock[$countStockOrPrice];
                        $countStockOrPrice += 1;
                    }
                }

                Item::where(['id' => $idItem])->update(['total_stock' => $total_stock]);
